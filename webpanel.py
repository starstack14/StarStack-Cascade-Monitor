...
<style>:root{--bg:#0c080d;--card:#21131b;--alt:#2b1722;--text:#fff5f1;--muted:#ae8e96;--green:#62e6a7;--red:#ff5548;--orange:#ffad5c;--cyan:#75d8ff;--border:#4b2833}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px system-ui;background-image:radial-gradient(circle at 50% -20%,#4d182c 0,transparent 43%)}main{max-width:760px;margin:auto;padding:15px}.head{display:flex;align-items:center;justify-content:space-between;padding:15px 4px 20px}.brand b{font-size:20px}.brand small{display:block;color:var(--muted);font-size:9px;letter-spacing:1.5px;margin-top:3px}.status{font-weight:700;background:#1a1016;padding:7px 10px;border-radius:9px}.ok{color:var(--green)}.bad{color:var(--red)}.muted{color:var(--muted)}.route{display:grid;grid-template-columns:1fr auto 1fr auto 1fr;gap:7px;align-items:center;margin-bottom:11px}.pill,.card{background:#21131be8;backdrop-filter:blur(14px);border:1px solid var(--border);border-radius:13px;box-shadow:0 12px 34px #0003}.pill{text-align:center;padding:12px 5px;font-size:12px}.arrow{color:var(--orange)}.card{padding:15px;margin:9px 0}.row{display:flex;justify-content:space-between;gap:10px;align-items:center}.title{font-size:16px;font-weight:700}.details{color:var(--muted);font-size:12px;margin-top:8px;line-height:1.7}.ip{color:var(--cyan);font-family:ui-monospace,monospace;cursor:pointer}.section{margin:20px 2px 8px;color:var(--muted);font-size:10px;font-weight:800;letter-spacing:1px}.device,.user{background:var(--alt);padding:12px;border-radius:10px;margin-top:7px;border:1px solid #4b283377}.badge{padding:3px 7px;border-radius:8px;background:var(--alt);color:var(--orange);font-size:11px}.empty{color:var(--muted);padding:18px;text-align:center}.footer{text-align:center;color:var(--muted);font-size:11px;padding:18px}a{color:var(--muted)}@media(max-width:520px){main{padding:11px}.route{gap:4px}.pill{padding:10px 2px;font-size:10px}.arrow{font-size:10px}.card{padding:13px}.title{font-size:14px}}</style></head><body><main><div class="head"><div class="brand"><b>◇ StarStack</b><small>CASCADE CONTROL · v2.3</small></div><div><span id="overall" class="status muted">● ЗАГРУЗКА</span></div></div><div class="route"><div id="p-router" class="pill">● NX31</div><div class="arrow">▶</div><div id="p-moscow" class="pill">● MOSCOW</div><div class="arrow">▶</div><div id="p-germany" class="pill">● GERMANY</div></div><div id="routes" class="card"></div><div id="router" class="card"></div><div class="section">НОДЫ</div><div id="nodes"></div><div class="section">ПОДКЛЮЧЕНЫ К VPN <span id="uc" class="badge">0</span></div><div id="users"></div><div class="section">ДОМАШНЯЯ СЕТЬ <span id="dc" class="badge">0</span></div><div id="devices"></div><div class="footer"><span id="updated">—</span> · <a href="/logout">выйти</a></div></main><script>
let hideIps=localStorage.getItem('starstack_hide_ips')==='1';let lastActivity=0;const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const ip=s=>hideIps?'•••.•••.•••.•••':esc(s);const cls=b=>b?'ok':'bad';const rate=n=>n==null?'—':(n*8/1e6).toFixed(2)+' Мбит/с';const bytes=n=>{if(!n)return '0 Б';let u=['Б','КБ','МБ','ГБ','ТБ'],i=0;while(n>=1024&&i<4){n/=1024;i++}return n.toFixed(1)+' '+u[i]};function activity(){let now=Date.now();if(now-lastActivity<15000)return;lastActivity=now;fetch('/api/activity',{method:'POST',credentials:'same-origin'})}['pointerdown','keydown','touchstart','scroll'].forEach(e=>addEventListener(e,activity,{passive:true}));document.addEventListener('click',e=>{activity();if(e.target.closest('.ip')){hideIps=!hideIps;localStorage.setItem('starstack_hide_ips',hideIps?'1':'0');update()}});
async function update(){try{let r=await fetch('/api/status',{cache:'no-store'});if(r.status===401){location='/';return}let d=await r.json();let good=d.healthy;overall.textContent=good?'● КАСКАД OK':'● НУЖНА ПРОВЕРКА';overall.className='status '+cls(good);let node=n=>d.nodes.find(x=>x.name===n)||{};let m=node('Moscow'),g=node('Germany');[['p-router',d.router.online&&d.router.singbox],['p-moscow',m.online],['p-germany',g.online]].forEach(x=>document.getElementById(x[0]).className='pill '+cls(x[1]));routes.innerHTML='<div class="row"><b>МАРШРУТЫ</b><span class="'+cls(d.routes.healthy)+'">'+(d.routes.healthy?'OK':'CHECK')+'</span></div><div class="details ip">DIRECT '+ip(d.routes.direct)+' · MOSCOW '+ip(d.routes.moscow)+' · GERMANY '+ip(d.routes.germany)+'</div>';router.innerHTML='<div class="row"><span class="title">▰ '+esc(d.router.hostname)+'</span><b class="'+cls(d.router.online)+'">'+(d.router.online?'ONLINE':'OFFLINE')+'</b></div><div class="details">sing-box '+(d.router.singbox?'●':'✕')+' · Load '+esc(d.router.load)+' · RAM '+esc(d.router.ram)+'<br><span class="ip">WAN '+ip(d.router.wan)+'</span></div>';nodes.innerHTML=d.nodes.map(n=>'<div class="card"><div class="row"><span class="title">'+esc(n.flag)+' '+esc(n.name)+'</span><b class="'+cls(n.online)+'">'+(n.online?'ONLINE':'OFFLINE')+'</b></div><div class="details">'+esc(n.latency)+' · Load '+esc(n.load)+' · RAM '+esc(n.ram)+' · 👤 '+esc(n.users)+'<br>Трафик '+bytes(n.traffic)+'</div></div>').join('')||'<div class="empty">Нет данных</div>';uc.textContent=d.users.length;users.innerHTML=d.users.map(u=>'<div class="user"><div class="row"><b>'+esc(u.name)+'</b><span class="ok">'+esc(u.node)+'</span></div><div class="details">'+esc(u.device)+' · <span class="ip">'+ip(u.ip)+'</span></div></div>').join('')||'<div class="empty">Нет активных подключений</div>';dc.textContent=d.devices.length;devices.innerHTML=d.devices.map(x=>'<div class="device"><div class="row"><b>'+(x.trusted?'✓ ':'')+esc(x.name)+'</b><span class="'+(x.blocked?'bad':'ok')+'">'+(x.blocked?'BLOCKED':esc(x.state))+'</span></div><div class="details"><span class="ip">'+ip(x.ip)+'</span> · '+esc(x.connection)+' · '+esc(x.signal)+'<br>Сейчас ↓ '+rate(x.rx_rate)+' / ↑ '+rate(x.tx_rate)+' · Месяц '+bytes(x.rx_total)+' / '+bytes(x.tx_total)+'</div></div>').join('')||'<div class="empty">Устройства не обнаружены</div>';updated.textContent='Обновлено '+new Date(d.updated*1000).toLocaleTimeString();}catch(e){overall.textContent='● НЕТ СВЯЗИ С ПРИЛОЖЕНИЕМ';overall.className='status bad'}}update();setInterval(update,5000);</script></body></html>"""
...
class DashboardWebServer:
    ...
    def start(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _session(self, touch: bool = False) -> str | None:
                ...
                now = time.time()
                if session:
                    expires, last_activity = session
                else:
                    expires = last_activity = 0
                if expires > now and now - last_activity <= owner.idle_seconds:
                    if touch:
                        owner.sessions[token] = (expires, now)
                    return token
            ...
            def do_POST(self) -> None:
                ...
                if path == "/api/activity":
                    if self._session(touch=True):
                        self._send(b"{}", 200, "application/json")
                    else:
                        self._send(b"{}", 401, "application/json")
                    return
                if path != "/login":
                ...
                owner.sessions[token] = (now + 86400, now)
                self.send_response(303)
